...
include_once '/protected/models/Flora.php';
...
class FloraController extends Controller
{
	...
	{ 
            if (Yii::app()->request->isAjaxRequest) {
                ...
                $data = Flora::model()->getData(array($f1), $s_date, $e_date);
	...
	public function actionChartMultiData()
	{
            if (Yii::app()->request->isAjaxRequest) {
                ...
                $data = Flora::model()->getData(array($f1, $f2), $s_date, $e_date);
                $marker = Checkpoints::model()->findByPk($id);
	...
	public function actionTableData()
	{
             if (Yii::app()->request->isAjaxRequest) {
                ...
                $this->renderPartial('TableData', array(
                    'dataProvider' => Flora::model()->getTableData($mon, $year)), FALSE, TRUE);
                Yii::app()->end();
            } else {
                Yii::app()->params['side_labels'] = Flora::model()->attributeLabels(FALSE);
                Yii::app()->params['side_year_list'] = Flora::model()->getYearsList();
                Yii::app()->params['side_month_list'] = Flora::model()->getMonthsList();

                $this->render('TableData', array(
                    'dataProvider' => Flora::model()->getTableData()));
            }
	}
