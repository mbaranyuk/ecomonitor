...
class AirController extends Controller
{
	...
	{ 
            if (Yii::app()->request->isAjaxRequest) {
                ...
                $data = Air::model()->getData(array($f1), $s_date, $e_date);
	...
	public function actionChartMultiData()
	{
            if (Yii::app()->request->isAjaxRequest) {
                ...
                $data = Air::model()->getData(array($f1, $f2), $s_date, $e_date);
	...
	public function actionTableData()
	{
             if (Yii::app()->request->isAjaxRequest) {
                ...
                $this->renderPartial('TableData', array(
                    'dataProvider' => Air::model()->getTableData($mon, $year)), FALSE, TRUE);
                Yii::app()->end();
            } else {
                Yii::app()->params['side_labels'] = Air::model()->attributeLabels(FALSE);
                Yii::app()->params['side_year_list'] = Air::model()->getYearsList();
                Yii::app()->params['side_month_list'] = Air::model()->getMonthsList();

                $this->render('TableData', array(
                    'dataProvider' => Air::model()->getTableData()));
            }
	}
