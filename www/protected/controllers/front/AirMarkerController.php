...
class AirMarkerController extends Controller {
    ...
    public function actionTableData() {
        ...
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $this->renderPartial('tableData', array('marker' => $marker,
                'dataProvider' => Air::model()->getMarkerTableData($this->marker_id, $mon, $year)), FALSE, TRUE);
            Yii::app()->end();
            
        } else {
            Yii::app()->params['side_labels'] = Air::model()->attributeLabels(FALSE);
            Yii::app()->params['side_year_list'] = Air::model()->getYearsList($this->marker_id);
            Yii::app()->params['side_month_list'] = Air::model()->getMonthsList($this->marker_id);

            $this->render('tableData', array('marker' => $marker,
                'dataProvider' => Air::model()->getMarkerTableData($this->marker_id)));
        }
    }
    ...
    public function actionChartData() {
        ...
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $data = Air::model()->getMarkerData($id, array($f1), $s_date, $e_date);
    ...
    public function actionChartMultiData() {
        ...
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $data = Air::model()->getMarkerData($id, array($f1, $f2), $s_date, $e_date);
            $marker = Checkpoints::model()->findByPk($id);
